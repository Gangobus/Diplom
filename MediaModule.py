...
class Sound(BoxLayout):
            ...
            if file_path_music:
                ...
                if sound:
                    ...
                    if file_path_music.lower().endswith(".wav"):
                        ...
                        musicfilediscr = f"{sample_rate} Гц, {channels} каналов, {bit_depth}-бит"
                    elif file_path_music.lower().endswith(".m4a"):
                        audio = MP4(file_path_music)
                        duration = audio.info.length
                        sample_rate = audio.info.sample_rate
    ...
    def on_seek_slider_value(self, instance, value):
        print('on seek slider value')
        if self.current_sound and self.current_sound.state == 'play':
            pos = value * self.current_sound.length
            self.current_sound.seek(pos)

    def stop_music_full(self):
        print('stop music full')
        if self.current_sound and self.play_pause_status == True:
            self.play_pause_status = False
    ...
    def play_video(self):
        try:
            ...
            self.video_player = VideoPlayer(source=video_file_path)
    ...
    def on_stop(self):
        ...
        self.video_player_position = None

    # метод приостановки воспроизведения видео
    def on_pause(self):
        self.video_player.state = 'pause'
        self.video_player_position = self.video_player.position
